...
import com.rokid.cxrmsamples.dataBeans.selfView.LottieAnimProps
import com.rokid.cxrmsamples.dataBeans.selfView.LottieRepeatMode
...
// 文档参考：08自定义页面（openCustomView、updateCustomView、closeCustomView、setCustomViewListener、sendCustomViewIcons）
class CustomViewViewModel: ViewModel() {
        ...
        children = listOf(
            ...
            SelfViewJson().apply {
                ...
                props = LottieAnimProps().apply {
                    ...
                    loop = true
                    repeatCount = 1
                    repeatMode = LottieRepeatMode.RESTART.str
                    speed = 1.0f
                    scale = 1.0f
                    progress = 0f
                }.toJson()
    ...
    /** 文档 08：openCustomView(selfViewJson) / closeCustomView() */
    ...
    fun updateCustomView() {
        ...
        // 文档 08：updateCustomView(updateViewJson)
    ...
    fun uploadIcon(context: Context) {
        ...
        uploadLottie(context)
    }

    private fun uploadLottie(context: Context) {
        // 加载Lottie动画
        val lottieIS = context.resources.openRawResource(R.raw.wave_loop)
        // 从InputString中读取Lottie动画 Json数据
        val lottieJson = try {
            val reader = BufferedReader(InputStreamReader(lottieIS))
        ...
        when(CxrApi.getInstance().sendCustomView_LottieAnimJson("lottieView", lottieJson)){
            ValueUtil.CxrStatus.REQUEST_SUCCEED -> {
                Log.d(TAG, "uploadLottie: Lottie动画发送成功")
            }
            ValueUtil.CxrStatus.REQUEST_WAITING -> {
                Log.e(TAG, "uploadLottie: Lottie动画发送失败")
            }
            else -> {
                Log.e(TAG, "uploadLottie: Lottie动画发送失败")
            }
    ...
    fun controlLottieAnim(controlType: CxrApi.LottieAnimControl){
        when(CxrApi.getInstance().sendCustomView_LottieAnimControl("lottieView", controlType)){
            ValueUtil.CxrStatus.REQUEST_SUCCEED -> {
                Log.d(TAG, "controlLottieAnim: Lottie动画控制成功")
            }
            ValueUtil.CxrStatus.REQUEST_WAITING -> {
                Log.e(TAG, "controlLottieAnim: Lottie动画控制失败,但不需要重试")
            }
            else -> {
                Log.e(TAG, "controlLottieAnim: Lottie动画控制失败")
            }
        }
    }
